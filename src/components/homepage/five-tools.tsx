...
const OLD_STACK = [
  { label: "PDF editor", issue: "Breaks layout & margins" },
  { label: "Word", issue: "Endless re-exports & version chaos" },
  { label: "Email", issue: "Feedback lost in reply chains" },
  { label: "eSign portal", issue: "Extra logins & per-envelope fees" },
  { label: "Cloud drive", issue: "Zero control over sent links" },
];

const NEW_FEATURES = [
  { title: "Native PDF editing", body: "Fix text without layout breaks" },
  { title: "Single live document", body: "No re-exports, no version chaos" },
  { title: "In-app team notes", body: "Threaded comments on the document" },
  { title: "Built-in signatures", body: "Sign in place, with no envelope fees" },
  { title: "1-click revocation", body: "Kill access anytime, anywhere" },
];
        ...
        <Reveal className="grid grid-cols-[minmax(0,1fr)_108px_minmax(0,1fr)] items-center">
          <div className="border border-ink/[0.14] bg-[#FCFCFD]">
            ...
            {OLD_STACK.map((row, i) => (
              <div
                key={row.label}
                className={`grid h-24 grid-cols-[20px_minmax(0,130px)_minmax(0,1fr)] items-center gap-4 px-6 ${
            ...
            <div className="relative border border-[#2C4453] bg-ink shadow-[0_34px_68px_rgba(14,36,51,0.34),0_0_44px_rgba(21,168,206,0.18)]">
              ...
              <div className="px-6 pb-2 pt-1">
                {NEW_FEATURES.map((f, i) => (
                  <div
                    key={f.title}
                    className={`grid grid-cols-[20px_minmax(0,1fr)] items-start gap-3.5 py-[17px] ${
