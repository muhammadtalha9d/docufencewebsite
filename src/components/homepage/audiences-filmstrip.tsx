      ...
      <div className="mx-auto flex max-w-[1180px] items-end justify-between gap-10 px-10 pb-12 pt-24">
        ...
        <div className="flex flex-shrink-0 gap-2.5">
          <button
            ...
            aria-label="Previous"
            onClick={() => scroll(-1)}
            className="flex h-11 w-11 items-center justify-center border border-mist/[0.28] text-mist hover:border-accent hover:text-accent"
          >
            ←
          </button>
          <button
      ...
      >
        {SLIDES.map((slide) => (
          <Link
            key={slide.href}
            href={slide.href}
            className="relative block h-[520px] flex-none w-[620px] overflow-hidden border border-mist/10 text-mist hover:border-accent/60"
            style={{
